...
function Qiniu_Sign($url) {//$info里面的url
    ...
    $duetime = NOW_TIME + 5*60 * 60 * 1000;//下载凭证有效时间
...
//获取七牛上传凭证
function Qiniu_upToken(){
    $setting = C ( 'UPLOAD_SITEIMG_QINIU' );
    $token['scope'] = "qiguovideo";
    $token['deadline'] = NOW_TIME + 5 * 60 * 60 * 1000;//设置失效时间为5小时后
//    $token['persistentOps'] = "avthumb/flv";
//    $token['persistentPipeline'] = "BingoStudyVideo";
    $json = json_encode($token);//对当前的token序列化成为json格式
    $encoded = Qiniu_Encode($json);
    $encoded_signed = Qiniu_Encode(hash_hmac('sha1',$encoded,$setting ["driverConfig"] ["secrectKey"],true));
    $uploadToken = $setting ["driverConfig"] ["accessKey"] . ':' . $encoded_signed . ':' . $encoded;
    return $uploadToken;
}
